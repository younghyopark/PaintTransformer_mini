    ...
    with torch.no_grad():
        ...
        for repeat in tqdm(range(repeat_num)):
            ...
            for j in range(original_img.shape[0]):
                save_img(final_result[j], frame_dir, "{:02d}_{}_current.png".format(repeat,j))
                save_img(original_img[j], frame_dir, "{:02d}_{}_target.png".format(repeat,j))
            ...
            for j in range(original_img.shape[0]):
                save_img(final_result[j], frame_dir, "{:02d}_{}_predicted.png".format(repeat,j))
            ...
            save_img(final_result[0], frame_dir, "{:02d}_predicted_resized_intermediate.png".format(repeat))
            # final_result = final_result.reshape(1, 4, 256, 256)
            original_img = make_grid(original_img, nrow=2**repeat, padding=0).unsqueeze(0)
            
            if repeat%1==0:
                save_img(final_result[0], frame_dir, "{:02d}_final.png".format(repeat))
...
if __name__ == '__main__':
    ...
    output_dir = './output/cropped_testing14'
