...
def main(input_path, model_path, output_dir, need_animation=False, resize_h=None, resize_w=None, serial=False):
    ...
    device="cpu"
    net_g = network.Painter_nongenerative(5, stroke_num, 256, 8, 3, 3).to(device)
    ...
    with torch.no_grad():
        ...
        for layer in range(0, K + 1):
            ...
            for j in range(img_patch.shape[0]):
                save_img(img_patch[j], 'checking/layer_{}_target_{}_{}.jpg'.format(layer,idx,j))
                save_img(result_patch[j], 'checking/layer_{}_current_{}_{}.jpg'.format(layer,idx,j))
            ...
            save_img(final_result[0], 'checking/layer_{}_prediction.jpg'.format(layer))
        ...
        shape_param, stroke_decision = net_g(img_patch, result_patch)
        for j in range(img_patch.shape[0]):
            save_img(img_patch[j], 'checking/layer_final_target_{}_{}.jpg'.format(idx,j))
            save_img(result_patch[j], 'checking/layer_final_current_{}_{}.jpg'.format(idx,j))
        grid = shape_param[:, :, :2].view(img_patch.shape[0] * stroke_num, 1, 1, 2).contiguous()
        img_temp = img_patch.unsqueeze(1).contiguous().repeat(1, stroke_num, 1, 1, 1).view(
            img_patch.shape[0] * stroke_num, 3, patch_size, patch_size).contiguous()
        color = F.grid_sample(img_temp, 2 * grid - 1, align_corners=False).view(
...
if __name__ == '__main__':
    main(input_path='../picture/jennifer.jpg',
